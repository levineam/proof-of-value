...
Blockchain consensus algorithms are technical infrastructure. They secure a ledger and meter its resources by charging for transactions, which aligns a financial system well and explains why, more than a decade in, finance remains nearly the only thing decentralization has produced. This paper proposes that the missing piece is a kind of **social consensus algorithm**: a decentralized method for leveraging the wisdom of the crowd to value an arbitrary piece of information and distribute newly issued tokens according to that valuation. Its requirements are constitutive rather than desirable—free to the participant, no expertise, no barriers to entry—because a network that charges for every small act of participation cannot collect the ordinary judgments it depends on.
...
Steem pioneered such a mechanism in its "proof of brain," and it worked, before failures elsewhere in its design clouded people's minds as to the most innovative feature it contained. Section 3 returns to what that experiment did and did not demonstrate.
...
The two roles are described generically because the split, not the choice of provider, is what matters. In the reference implementation the social protocol is AT Protocol and the settlement chain is Koinos; §11 states what a substitute for either would have to provide.
...
## 14. Conclusion
What this paper ultimately describes is not a token and not an application. It is a marketplace any group can stand up to price the information its own members produce, according to its own idea of what is valuable. A fixed period budget, locked vote-linked stake, signed positive and negative evaluations, author-only allocations, and settlement-time unlock are the machinery; the point of the machinery is that a community can run it without asking anyone's permission and without a center to issue and enforce the instrument.

The reason to want many of these rather than one is the same reason a single universal ranking of human value would be worse than useless. A research collective, an open-source project, a trade association, and a neighborhood do not agree about what deserves reward, and no arbiter should have to adjudicate between them. Plural marketplaces make valuation local and legible, and they make capture survivable: a community that judges its own marketplace captured can leave and run another under the same rules. That is the sense in which this is infrastructure for decentralized organizations rather than a product with users—the useful thing is not any particular marketplace but the ability to create them.

Version one is one marketplace, not the factory, and it is deliberately small: a valueless token, a bounded prototype, and a set of open questions stated rather than answered. The mechanism is not neutral, objective, or immune to capital and social power. But if a community can direct a shared budget toward what its members actually value, using nothing more than an ordinary judgment that costs nothing to express, then the missing instrument is buildable—and everything after that is a question of whether people find it worth using.
## References
