...
> 
> **This is a beta, and the token is not the point.** The first SWARM token is a valueless test token. It is not designed to appreciate in value, and no one should participate expecting it to do so. The development authority explicitly retains the right to change the token economics entirely: Koinos's upgradeable contracts make rapid iteration the purpose of this stage rather than an exception to it. Every such change is announced and versioned, and the authority is subject to the sunset conditions in §9.
## Abstract
Proof of Value provides **turnkey marketplaces for information**: any organization can deploy one to reward the information its members produce, according to the judgment of the people who actually value it. The product is the marketplace, not a token.

Blockchain consensus algorithms are technical infrastructure. They secure a ledger and meter its resources by charging for transactions, which aligns incentives within a financial system and explains why, more than a decade in, finance remains nearly the only thing decentralization has produced. This paper proposes that the missing piece is a kind of **social consensus algorithm**: a decentralized method for using collective judgment to value an arbitrary piece of information and distribute newly issued tokens according to that valuation. Its requirements are constitutive rather than desirable—free to the participant, no expertise, no barriers to entry—because a network that charges for every small act of participation cannot collect the ordinary judgments it depends on.

The reference mechanism, SWARM, issues a per-period budget at a fixed inflation rate and allocates it at settlement by stake-weighted upvotes and downvotes, each vote committing and locking a fraction of the voter's balance. Content and identity remain on AT Protocol; settlement runs on Koinos, whose feeless model satisfies the requirements above without being the only chain that could.

Its standard is provable fairness, not correctness: the rules are published, applied identically, and reproducible from the public record. Nothing here is demonstrated yet. The token is a valueless test token, the prototype is a beta whose economics may change entirely, and the principal open questions—whether unpaid evaluation is sustainable, and whether stake-weighted judgment survives machine-generated content and machine-delegated voting—are stated rather than answered.
...
Decentralization was expected to extend beyond finance once the infrastructure existed: decentralized organizations, applications, and platforms would follow. They have not. Of the thousands of tokens launched daily, almost none belong to anything meaningfully decentralized, and the projects that tried to carry decentralization into social applications have now largely been abandoned. Each wave concludes that the problem lay somewhere else: in the product, the interface, or the market.
...
Steem pioneered such a mechanism in its “Proof of Brain.” Section 3 returns to what that experiment did and did not demonstrate.
...
A **social consensus algorithm** is a decentralized method for using collective judgment to value an arbitrary piece of information and to distribute newly issued tokens according to that valuation. It is the general form of what this paper argues is missing; SWARM is one candidate implementation of it. Three properties are constitutive rather than desirable: it must be free to the participant, require no specialized expertise, and impose no barriers to entry. A mechanism failing any of the three cannot collect the volume of ordinary judgments it needs to produce a meaningful signal, however sound its allocation rule.
...
The argument in Section 1 would be speculative if nothing had ever tried it. Something did. Steem ran a continuous, feeless, openly joinable mechanism of per-item upvotes and downvotes that allocated real issuance at social scale. It demonstrated that a community-evaluation mechanism could direct issuance without requiring each reader to make a payment. That is the narrow claim relevant here; it does not establish that the mechanism produced high-quality allocations, caused the network's growth, or was economically sustainable. This paper's central claim is not that such an algorithm is new. It is that the one system to demonstrate the category was never evaluated on its own terms.
...
for eligible contributions `j` with positive allocation weight. This is a SWARM reference formula, not an inherited or deployed default. If there are no eligible positive contributions, the treatment of `B_t` must be specified by policy rather than assumed here. Under the proposed nonnegative-allocation invariant, contributions with negative or zero scores receive no newly created SWARM and a downvote never confiscates previously earned balance; this invariant must be implemented and tested.

The version-one candidate for `F` is informed by Vandeberg's historical convergent-linear proposal and the later protected-downvote design. It is not a claim that current Hive uses that curve: the maintained Hive HF25 source sets author and curation curves to linear. An allocation curve is not merely a mechanical detail; it expresses a theory of fairness. A linear curve is proportional but vulnerable to stake-splitting; a superlinear curve resists stake-splitting but grows increasingly unfair to large positions; Vandeberg's convergent-linear form (`n²/(n+1)`) is an attempt to hold both properties at once. Most contributions attract little agreement, and scattered, isolated votes are closer to noise than signal. In its superlinear region, the convergent-linear curve gives low-consensus contributions practically nothing, so the budget is not spread thinly across everything; as independent evaluations accumulate on the same contribution, rewards grow steeply; and the curve's convergence to linear bounds that amplification, so a heavily voted contribution keeps earning proportionally but its advantage stops compounding. The curve is one proposed implementation of fair aggregation, not the reason the mechanism exists. SWARM's exact curve and constants remain proposed parameters requiring simulation and validation.
...
> **The social protocol owns identity, content, social context, and distribution. The settlement chain owns vote-linked token locks, algorithmic token issuance, deterministic reward accounting, and payouts.**

The two roles are described generically because the split, not the choice of provider, is what matters. In the reference implementation the social protocol is AT Protocol and the settlement chain is Koinos; §11 states what a substitute settlement chain would have to provide.
...
### Machine-generated content and machine-delegated evaluation

The most serious threat to this design is not a flaw in the allocation rule. It is that both sides of the mechanism can be automated.

On the supply side, the cost of producing plausible, competent content has fallen to near zero. The period budget is fixed while the population of candidates for it is not, and a curve that filters low-consensus contributions does not distinguish genuine consensus from manufactured consensus. On the evaluation side, nothing in the design requires that a judgment be made by a person. A holder can delegate voting to software that votes the full per-period capacity at whatever is most likely to attract issuance—and because version one pays nothing for honest evaluation, a participant who evaluates carefully without compensation competes directly against an automated participant optimizing for return. AT Protocol identities are not proofs of personhood, and nothing here establishes one.

The uncomfortable implication runs against a convenient story. It would be neat to argue that as automation absorbs measurable work, human judgment becomes more valuable and a mechanism for rewarding it becomes more timely. The opposite may hold: automation makes *trustworthy* judgment scarcer and harder to identify, and a mechanism resting on mass stake-weighted voting is exposed precisely where automation is strongest. Skin in the game is a weaker defense than it first appears, because capital can be applied to automated judgment as readily as to human judgment—stake does not certify that anyone thought about anything.

This design does not solve that. Three partial mitigations are available and none is sufficient alone: bounded contestability lets stakeholders push back on allocations that look manufactured (§7); marketplaces scoped to communities whose contributions border on verifiable outcomes give slop somewhere to fail before it reaches a vote (§10); and locked stake at least makes automated participation cost something. Whether that is enough is an empirical question and is stated as an open one (§13, question 13).
...
The prototype uses one valueless SWARM test token on a Koinos test network. Test networks are restartable, and their endpoints and chain IDs may change, so the implementation must retrieve and disclose the active chain ID at runtime rather than hard-code one or imply permanence.
...
The first prototype proposes to demonstrate real information hosted on AT Protocol, AT login, Koinos account linking, sponsored upvotes and downvotes, vote-linked locks, deterministic reward accounting, author-DID accrual, claims, and ranking inside the standalone PoV client. Returning that ranking to AT distribution surfaces is deferred. The intended boundary is a Koinos test network, using its then-current chain ID, and one valueless token.

**The first marketplace evaluates contributions to this project.** Rather than opening on general social content, the prototype's information set is the work of building Proof of Value itself: pull requests, issues, design documents, review comments, and written argument. Three reasons make this more than a matter of convenience. It is the smallest complete instance of the paper's own claim, so the first thing the mechanism is asked to do is the thing the paper says a decentralized organization needs. It supplies a population with a reason to evaluate honestly that does not depend on being paid for it, which is the assumption version one is making and cannot yet justify (§13, question 9). And it borders on ground truth: a contribution is merged or not, builds or not, is used or not, so a large share of low-quality submissions can be filtered before they ever reach a vote—a defense a general content feed does not have against cheap synthetic material (§8).

The claim this demonstration can support is correspondingly narrow. A project with a handful of contributors is close to the worst case for a mechanism whose entire premise is aggregating many independent vantages; with few voters, stake-weighting tends toward whoever holds the most, which exhibits the concentration failure of §8 rather than refuting it. Founders evaluating work they themselves benefit from is textbook self-dealing, and §7's contestability is the only check on it. This first marketplace should therefore be read as evidence that the loop completes end to end—attestation, evaluation, settlement, claim, all inspectable—and not as evidence that the mechanism produces good collective judgment. That requires more participants than this project has.
...
| Proof of Brain (Steem) | Stake-weighted continuous upvotes and downvotes over a fixed issuance | Continuous, per item | None | Open | Elected validator set, outside the reward rule |

Two observations follow. First, every one of these systems reintroduces a locus of judgment somewhere—a curated project list, an appointed reviewer set, an administrator, a hand-tuned weight, a governance whitelist. This paper does not escape that, and Sections 5, 8, and 9 identify its own such loci explicitly: the attestation bridge, the sponsor, the initial distribution, and the upgrade authority. A design that claims to have removed every center has usually just failed to look for its own.
...
A regenerating, stake-proportional resource allowance that makes participation free is the same idea as Steem's resource credits; the convergent allocation curve belongs to the same family as the curve Steem adopted in its later revisions. Neither is presented here as an invention.
...
The requirements are structural; the host is not. The mechanism requires a chain on which participation costs the user nothing, a sponsor can bear that cost within bounded limits, and the contracts remain upgradeable while the mechanism is still being learned. Koinos satisfies these directly, which is why it is the reference implementation and the first testbed. It is not the only chain that could: execution costs elsewhere have fallen far enough that sponsored, effectively free participation is becoming practical on several networks, and any chain meeting the three requirements is a candidate host. Nothing in SWARM's allocation rule, lock semantics, or event model depends on Koinos specifically, and the design should be read as portable by intent. The claim of this paper is about what the mechanism requires, not about which ledger provides it.
...
11. Is a vote-linked lock until settlement sufficient skin in the game? A vote immobilizes stake until settlement, but the voter bears no loss for having voted badly: nothing resolves, and no gain or loss attaches to the judgment itself, unlike a position in a prediction market. Whether locked opportunity cost alone is enough to produce careful evaluation is untested.
  
12. Does the revised mechanism actually suppress the failure modes that buried Steem? Vote-selling markets, self-dealing by large holders, and retaliatory downvoting are properties of stake-weighted continuous voting rather than of the chain beneath it (§3). The convergent curve, the vote-linked lock until settlement, and bounded downvote capacity are attempts to suppress them; none is yet demonstrated.
...
## 14. Conclusion
What this paper ultimately describes is not a token and not an application. It is a marketplace any group can stand up to price the information its own members produce, according to its own idea of what is valuable. A fixed period budget, vote-linked locks until settlement, signed positive and negative evaluations, author-only allocations, and settlement-time unlock are the machinery; the point of the machinery is that a community can run it without asking anyone's permission and without a center to issue and enforce the instrument.

Plural marketplaces make valuation local and legible, and they make capture survivable: a community that judges its marketplace captured can leave and run another under the same mechanism.

Version one tests a narrower claim: whether one project can complete an inspectable attestation, evaluation, settlement, and claim loop with a valueless token, and whether participants find a free, ordinary judgment worth making repeatedly. It does not demonstrate good collective judgment, immunity to capital or social power, or a finished marketplace factory. If the loop works and people use it, the missing instrument is buildable; everything after that is a question of whether communities find it worth operating.
## References
